...
class StencilConvertAST(ast_tools.ConvertAST):
    ...
    def visit_ScalarBinOp(self, node):
        # "BUGGY" TRANSLATION FROM PYTHON TO C++
        return super(StencilConvertAST, self).visit_BinOp(ast.BinOp(node.left, ast.Sub(), node.right))

        # "CORRECT" TRANSLATION FROM PYTHON TO C++
